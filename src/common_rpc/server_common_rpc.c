...
Rpc__AcceptedReply *create_default_case_accepted_reply(Rpc__AcceptStat default_case_accept_stat) {
    if (default_case_accept_stat == RPC__ACCEPT_STAT__SUCCESS ||
        default_case_accept_stat == RPC__ACCEPT_STAT__PROG_MISMATCH) {
        return NULL;
    }
...
void free_accepted_reply(Rpc__AcceptedReply *accepted_reply) {
    ...
    if (accepted_reply->stat == RPC__ACCEPT_STAT__SUCCESS) {
        ...
        // for all other AcceptStat's - PROG_UNAVAIL, GARBAGE_ARGS, SYSTEM_ERR, etc. we only need to free the Empty
        // default_case
