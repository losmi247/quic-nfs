...
*/
int receive_bytes(int socket_fd, size_t bytes_to_read, uint8_t *destination_buffer) {
...
*/
uint8_t *receive_rm_record(int socket_fd, size_t *rm_record_data_size) {
    ...
    while(!is_last_fragment) {
        ...
        error_code = receive_bytes(socket_fd, fragment_size, rm_record_data_buffer + rm_record_data_bytes_read);
        if(error_code > 0) {
            free(rm_record_data_buffer);
            return NULL;
        }
...
* Returns 0 on success and > 0 on failure.
*/
int send_bytes(int socket_fd, const uint8_t *source_buffer, size_t buffer_size) {
...
int send_rm_record(int socket_fd, const uint8_t *rm_record_data, size_t rm_record_data_size) {
    ...
    while(rm_record_data_bytes_sent < rm_record_data_size) {
        ...
        int error_code = send_bytes(socket_fd, header_buffer, sizeof(fragment_header));
        if(error_code > 0) {
            return 1;
        }
        
        // send the fragment data
        error_code = send_bytes(socket_fd, rm_record_data + rm_record_data_bytes_sent, fragment_size);
        if(error_code > 0) {
            return 2;
        }
