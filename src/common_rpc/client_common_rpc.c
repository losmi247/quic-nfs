...
int validate_rpc_reply_structure(Rpc__RpcMsg *rpc_reply) {
    ...
    if (reply_body->stat == RPC__REPLY_STAT__MSG_DENIED) {
        if (reply_body->reply_case != RPC__REPLY_BODY__REPLY_RREPLY) {
            fprintf(stderr,
                    "Something went wrong at server - inconsistent ReplyStat and reply_case in received RPC reply.\n");
            return 4;
        }
        ...
        case RPC__REJECT_STAT__RPC_MISMATCH:
            if (rejected_reply->reply_data_case != RPC__REJECTED_REPLY__REPLY_DATA_MISMATCH_INFO) {
                fprintf(stderr, "Something went wrong at server - inconsistent RejectStat and reply_data_case in "
                                "received RPC reply.\n");
                return 6;
            }
    ...
    if (verifier->flavor == RPC__AUTH_FLAVOR__AUTH_NONE) {
        ...
        if (verifier->empty == NULL) {
            fprintf(stderr, "Something went wrong at server - OpaqueAuth flavor is AUTH_NONE, but NULL "
                            "OpaqueAuth->empty, in received RPC reply.\n");
            return 12;
        }
    }
    // TODO (QNFS-52): Implement AUTH_SHORT
        ...
        fprintf(
            stderr,
            "Something went wrong at server - verifier flavor is %d which is not supported, in received RPC reply.\n",
            verifier->flavor);
        return 13;
    }
    ...
    case RPC__ACCEPT_STAT__SUCCESS:
        if (accepted_reply->reply_data_case != RPC__ACCEPTED_REPLY__REPLY_DATA_RESULTS) {
            fprintf(stderr, "Something went wrong at server - inconsistent AcceptStat and reply_data_case in received "
                            "RPC reply.\n");
            return 14;
        }
...
int validate_successful_accepted_reply(Rpc__RpcMsg *rpc_reply) {
    ...
    case RPC__ACCEPT_STAT__PROG_MISMATCH:
        Rpc__MismatchInfo *mismatch_info = accepted_reply->mismatch_info;
        fprintf(stderr,
                "Accepted RPC Reply: requested RPC program version not supported, min version=%d, max version=%d\n",
                mismatch_info->low, mismatch_info->high);
        return 4;
    default:
