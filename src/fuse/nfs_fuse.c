...
static struct fuse_operations nfs_oper = {.init = nfs_fuse_init,
...
int mount_remote_filesystem(char *server_ipv4_address, uint16_t server_port, char *remote_absolute_path) {
    ...
    rpc_connection_context =
        create_auth_sys_rpc_connection_context(server_ipv4_address, server_port, chosen_transport_protocol);
...
int main(int argc, char *argv[]) {
    if (argc != 6) {
        fprintf(stderr,
                "Error: Incorrect usage. Correct usage: %s <IPv4 addr> <port number> --proto=<tcp or quic> <remote "
    ...
    // don't let FUSE decide when it wants to retry file system operations - allow NFS a long time of 60 seconds to
    // reply
