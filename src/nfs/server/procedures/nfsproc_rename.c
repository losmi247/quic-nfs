 ...
 */
Rpc__AcceptedReply *serve_nfs_procedure_11_rename_file(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier,
    ...
    if (error_code > 0) {
        ...
        fprintf(stderr,
                "serve_nfs_procedure_11_rename_file: failed getting file/directory attributes for file/directory at "
    ...
    if (from_directory_fattr.nfs_ftype->ftype != NFS__FTYPE__NFDIR) {
        ...
        fprintf(
            stderr,
            "serve_nfs_procedure_11_rename_file: 'rename' procedure called with 'from' being a non-directory '%s'\n",
    ...
    if (error_code < 0) {
        if (errno == ENOENT) {
            ...
            case ENOENT:
                ...
                fprintf(stderr,
                        "serve_nfs_procedure_11_rename_file: attempted to rename a file '%s' that does not exist\n",
            ...
            perror_msg("serve_nfs_procedure_11_rename_file: failed checking if file to be renamed at absolute path "
                       "'%s' already exists",
                       old_file_absolute_path);
    ...
    if (to_directory_fattr.nfs_ftype->ftype != NFS__FTYPE__NFDIR) {
        // if the file is not a directory, return NfsStat with 'non-directory specified in a directory operation' status
        fprintf(stderr,
                "serve_nfs_procedure_11_rename_file: 'rename' procedure called with 'to' being a non-directory '%s'\n",
    ...
    if (credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat = check_rename_proc_permissions(from_directory_absolute_path, to_directory_absolute_path,
                                                 credential->auth_sys->uid, credential->auth_sys->gid);
        if (stat < 0) {
            fprintf(stderr,
                    "serve_nfs_procedure_11_rename_file: failed checking RENAME permissions for moving a file at "
                    "absolute path '%s' to '%s/%s' with error code %d\n",
    ...
    if (error_code < 0) {
        if (errno == EDQUOT || errno == EINVAL || errno == ENAMETOOLONG || errno == ENOENT || errno == ENOSPC ||
            errno == ENOTEMPTY || errno == EROFS) {
            Nfs__Stat nfs_stat;
            switch (errno) {
            case EDQUOT:
                nfs_stat = NFS__STAT__NFSERR_DQUOT;
                fprintf(stderr,
                        "serve_nfs_procedure_11_rename_file: the user's quota of disk blocks on the file system has "
                        "been exhausted when trying to rename file at absolute path '%s' to '%s'\n",
                        old_file_absolute_path, new_file_absolute_path);
                break;
            case EINVAL:
                nfs_stat = NFS__STAT__NFSERR_EXIST;
                fprintf(stderr,
                        "serve_nfs_procedure_11_rename_file: an attempt was made to make a directory a subdirectory of "
                        "itself when trying to rename file at absolute path '%s' to '%s'\n",
                        old_file_absolute_path, new_file_absolute_path);
                break;
            case ENAMETOOLONG:
                nfs_stat = NFS__STAT__NFSERR_NAMETOOLONG;
                fprintf(stderr,
                        "serve_nfs_procedure_11_rename_file: attempted to rename a file at absolute path '%s' to '%s', "
                        "one of which exceeds system limit on pathname length\n",
                        old_file_absolute_path, new_file_absolute_path);
                break;
            case ENOENT:
                nfs_stat = NFS__STAT__NFSERR_NOENT;
                fprintf(stderr,
                        "serve_nfs_procedure_11_rename_file: a directory component in new absolute path '%s' does not "
                        "exist\n",
                        new_file_absolute_path);
                break;
            case ENOSPC:
                nfs_stat = NFS__STAT__NFSERR_NOSPC;
                fprintf(stderr, "serve_nfs_procedure_11_rename_file: the device containing the file has no room for "
                                "the new directory entry\n");
                break;
            case ENOTEMPTY:
                nfs_stat = NFS__STAT__NFSERR_NOTEMPTY;
                fprintf(stderr,
                        "serve_nfs_procedure_11_rename_file: attempted to move file at absolute path '%s' to absolute "
                        "path '%s' which is a nonempty directory\n",
                        old_file_absolute_path,
                        new_file_absolute_path); // not allowed to overwrite nonempty directories!
                break;
            case EROFS:
            ...
            perror_msg("serve_nfs_procedure_11_rename_file: failed to 'rename' the file at absolute path '%s' to "
                       "absolute path '%s'\n",
                       old_file_absolute_path, new_file_absolute_path);
    ...
    error_code = update_inode_mapping_absolute_path_by_absolute_path(old_file_absolute_path, new_file_absolute_path,
                                                                     &inode_cache);
    if (error_code > 1) {
        perror_msg("serve_nfs_procedure_11_rename_file: failed to update the inode mapping for file at absolute path "
                   "'%s' being moved to absolute path '%s'\n",
                   old_file_absolute_path, new_file_absolute_path);
