...
Rpc__AcceptedReply *serve_nfs_procedure_1_get_file_attributes(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier, Google__Protobuf__Any *parameters) {
    ...
    if(credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat = check_getattr_proc_permissions(file_absolute_path, credential->auth_sys->uid, credential->auth_sys->gid);
        if(stat < 0) {
            fprintf(stderr, "serve_nfs_procedure_1_get_file_attributes: failed checking GETATTR permissions for file/directory at absolute path '%s' with error code %d\n", file_absolute_path, stat);
