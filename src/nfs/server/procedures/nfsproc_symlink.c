 ...
 */
Rpc__AcceptedReply *serve_nfs_procedure_13_create_symbolic_link(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier,
    ...
    if (directory_absolute_path == NULL) {
        ...
        fprintf(stderr,
                "serve_nfs_procedure_13_create_symbolic_link: failed to decode inode number %ld back to a directory\n",
                inode_number);
    ...
    if (error_code > 0) {
        ...
        fprintf(stderr,
                "serve_nfs_procedure_13_create_symbolic_link: failed getting file/directory attributes for "
    ...
    if (directory_fattr.nfs_ftype->ftype != NFS__FTYPE__NFDIR) {
        ...
        fprintf(stderr,
                "serve_nfs_procedure_13_create_symbolic_link: 'symlink' procedure called on a non-directory '%s'\n",
    ...
    if (strlen(file_name->filename) > NFS_MAXNAMLEN) {
        fprintf(stderr,
                "serve_nfs_procedure_13_create_symbolic_link: attempted to create a symbolic link in directory '%s' "
                "with file name longer than NFS limit\n",
                directory_absolute_path);
    ...
    if (error_code == 0) {
        fprintf(stderr,
                "serve_nfs_procedure_13_create_symbolic_link: attempted to create a symbolic link at '%s' but file at "
        ...
        perror_msg("serve_nfs_procedure_13_create_symbolic_link: failed checking if symbolic link to be created at "
                   "absolute path '%s' is a file that already exists",
                   file_absolute_path);
    ...
    if (credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat = check_symlink_proc_permissions(directory_absolute_path, credential->auth_sys->uid,
                                                  credential->auth_sys->gid);
        if (stat < 0) {
            fprintf(stderr,
                    "serve_nfs_procedure_13_create_symbolic_link: failed checking SYMLINK permissions for creating a "
                    "symbolic link at absolute path '%s' to target '%s' with error code %d\n",
    ...
    if (error_code < 0) {
        if (errno == EIO || errno == ENAMETOOLONG || errno == ENOENT || errno == ENOSPC || errno == EROFS) {
            ...
            case EIO:
                ...
                fprintf(stderr,
                        "serve_nfs_procedure_13_create_symbolic_link: physical IO error occurred while trying to "
                        "create a symbolic link at absolute path '%s' to target '%s'\n",
            ...
            case ENAMETOOLONG:
                ...
                fprintf(
                    stderr,
                    "serve_nfs_procedure_13_create_symbolic_link: attempted to create a symbolic link at absolute path "
            ...
            case ENOENT:
                ...
                fprintf(stderr,
                        "serve_nfs_procedure_13_create_symbolic_link: attempted to create a symbolic link at absolute "
                        "path '%s' to target '%s', and some directory component in '%s' does not exist or is a "
            ...
            case ENOSPC:
                ...
                fprintf(stderr,
                        "serve_nfs_procedure_13_create_symbolic_link: no space left to add a new entry '%s' to "
                        "directory '%s'\n",
            ...
            case EROFS:
                ...
                fprintf(stderr,
                        "serve_nfs_procedure_13_create_symbolic_link: attempted to create a symbolic link at '%s' on a "
            ...
            perror_msg("serve_nfs_procedure_13_create_symbolic_link: failed creating symbolic link at absolute path "
                       "'%s' to target '%s'\n",
                       file_absolute_path, to->path);
