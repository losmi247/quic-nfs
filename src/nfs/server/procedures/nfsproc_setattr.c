 ...
 */
Rpc__AcceptedReply *serve_nfs_procedure_2_set_file_attributes(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier,
    ...
    if (file_absolute_path == NULL) {
        ...
        fprintf(
            stderr,
            "serve_nfs_procedure_2_set_file_attributes: failed to decode inode number %ld back to a file/directory\n",
            inode_number);
    ...
    if (credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat = check_setattr_proc_permissions(file_absolute_path, sattr, credential->auth_sys->uid,
                                                  credential->auth_sys->gid);
        if (stat < 0) {
            fprintf(stderr,
                    "serve_nfs_procedure_2_set_file_attributes: failed checking SETATTR permissions for file/directory "
    ...
    if (sattr->mode != -1 && chmod(file_absolute_path, sattr->mode) < 0) {
        perror_msg("serve_nfs_procedure_2_set_file_attributes: failed to update 'mode' attribute of file/directory at "
                   "absolute path '%s'\n",
                   file_absolute_path);
    ...
    if (chown(file_absolute_path, sattr->uid, sattr->gid) <
        ...
        perror_msg("serve_nfs_procedure_2_set_file_attributes: failed to update 'uid' and 'gid' attributes of "
                   "file/directory at absolute path '%s'\n",
                   file_absolute_path);
    ...
    if (sattr->size != -1 && truncate(file_absolute_path, sattr->size) < 0) {
        perror_msg("serve_nfs_procedure_2_set_file_attributes: failed to update 'size' attributes of file/directory at "
                   "absolute path '%s'\n",
                   file_absolute_path);
    ...
    if (sattr->atime->seconds != -1 && sattr->atime->useconds != -1 && sattr->mtime->seconds != -1 &&
        ...
        if (utimes(file_absolute_path, times) < 0) {
            perror_msg("serve_nfs_procedure_2_set_file_attributes: failed to update 'atime' and 'mtime' attributes of "
                       "file/directory at absolute path '%s'\n",
                       file_absolute_path);
    ...
    if (error_code > 0) {
        fprintf(stderr,
                "serve_procedure_2_set_file_attributes: failed getting attributes for file/directory at absolute path "
                "'%s' with error code %d \n",
