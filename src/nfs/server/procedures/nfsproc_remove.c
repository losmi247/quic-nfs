...
Rpc__AcceptedReply *serve_nfs_procedure_10_remove_file(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier, Google__Protobuf__Any *parameters) {
    ...
    if(fattr.nfs_ftype->ftype == NFS__FTYPE__NFDIR) {
        // if the file is actually directory, return NfsStat with 'directory specified in a non-directory operation' status
    ...
    if(credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat = check_remove_proc_permissions(directory_absolute_path, credential->auth_sys->uid, credential->auth_sys->gid);
        if(stat < 0) {
            fprintf(stderr, "serve_nfs_procedure_10_remove_file: failed checking REMOVE permissions for removing a file at absolute path '%s' with error code %d\n", file_absolute_path, stat);
