...
Rpc__AcceptedReply *serve_nfs_procedure_12_create_link_to_file(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier, Google__Protobuf__Any *parameters) {
    ...
    if(target_file_absolute_path == NULL) {
        ...
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: failed to decode inode number %ld back to a file\n", target_file_inode_number);
    ...
    // get the attributes of the looked up file before creating a hard link to it, to check that the file is a regular file
    ...
    if(error_code > 0) {
        ...
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: failed getting attributes for file/directory at absolute path '%s' with error code %d\n", target_file_absolute_path, error_code);
    ...
    if(target_file_fattr.nfs_ftype->ftype != NFS__FTYPE__NFREG) {
        ...
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: file '%s' which is not a regular file was specified for 'link' which is only possible for regular files\n", target_file_absolute_path);
    ...
    if(directory_absolute_path == NULL) {
        ...
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: failed to decode inode number %ld back to a directory\n", inode_number);
    ...
    if(error_code > 0) {
        // we failed getting attributes for this file
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: failed getting file/directory attributes for file/directory at absolute path '%s' with error code %d\n", directory_absolute_path, error_code);
    ...
    if(strlen(file_name->filename) > NFS_MAXNAMLEN) {
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: attempted to create a hard link in directory '%s' with file name longer than NFS limit\n", directory_absolute_path);
    ...
    if(error_code == 0) {
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: attempted to create a hard link with at '%s' but file at that absolute path already exists\n", file_absolute_path);
        ...
        fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: physical IO error occurred while checking if file at absolute path '%s' exists\n", file_absolute_path);
        ...
        perror_msg("serve_nfs_procedure_12_create_link_to_file: failed checking if hard link to be created at absolute path '%s' is a file that already exists", file_absolute_path);
    ...
    if(credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat = check_link_proc_permissions(directory_absolute_path, credential->auth_sys->uid, credential->auth_sys->gid);
        if(stat < 0) {
            fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: failed checking LINK permissions for creating a hard link at absolute path '%s' to target '%s' with error code %d\n", file_absolute_path, target_file_absolute_path, stat);
    ...
    if(error_code < 0) {
        if(errno == EDQUOT|| errno == EIO || errno == ENAMETOOLONG || errno == ENOENT || errno == ENOSPC || errno == EROFS) {
            Nfs__Stat nfs_stat;
            switch(errno) {
                case EDQUOT:
                    nfs_stat = NFS__STAT__NFSERR_DQUOT;
                    fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: user's quota of disk blocks on the filesystem has been exhausted while trying to create a hard link at absolute path '%s' to target '%s'\n", file_absolute_path, target_file_absolute_path);
                    break;
                case EIO:
                    nfs_stat = NFS__STAT__NFSERR_IO;
                    fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: physical IO error occurred while trying to create a hard link at absolute path '%s' to target '%s'\n", file_absolute_path, target_file_absolute_path);
                    break;
                case ENAMETOOLONG:
                    nfs_stat = NFS__STAT__NFSERR_NAMETOOLONG;
                    fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: attempted to create a hard link at absolute path '%s' to target '%s', and one of these pathnames exceeds system limit on pathname length\n", file_absolute_path, target_file_absolute_path);
                    break;
                case ENOENT:
                    nfs_stat = NFS__STAT__NFSERR_NOENT;
                    fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: attempted to create a hard link at absolute path '%s' to target '%s', and some directory component in these two paths does not exist or is a dangling symbolic link\n", file_absolute_path, target_file_absolute_path);
                    break;
                case ENOSPC: 
                    nfs_stat = NFS__STAT__NFSERR_NOSPC;
                    fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: no space left to add a new entry '%s' to directory '%s'\n", file_absolute_path, directory_absolute_path);
                    break;
                case EROFS:
                    nfs_stat = NFS__STAT__NFSERR_ROFS;
                    fprintf(stderr, "serve_nfs_procedure_12_create_link_to_file: attempted to create a hard link at '%s' on a read-only filesystem\n", file_absolute_path);
                    break;
            }
            ...
            perror_msg("serve_nfs_procedure_12_create_link_to_file: failed creating hard link at absolute path '%s' to target '%s'\n", file_absolute_path, target_file_absolute_path);
