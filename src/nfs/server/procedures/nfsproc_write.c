 ...
 */
Rpc__AcceptedReply *serve_nfs_procedure_8_write_to_file(Rpc__OpaqueAuth *credential, Rpc__OpaqueAuth *verifier,
    ...
    if (error_code > 0) {
        fprintf(stderr,
                "serve_nfs_procedure_8_write_to_file: failed getting attributes for file/directory at absolute path "
                "'%s' with error code %d\n",
    ...
    if (fattr.nfs_ftype->ftype == NFS__FTYPE__NFDIR) {
        // if the file is actually directory, return AttrStat with 'directory specified in a non-directory operation'
        // status
        fprintf(stderr,
                "serve_nfs_procedure_8_write_to_file: a directory '%s' was specified for 'write' which is a "
    ...
    if (writeargs->nfsdata.len > NFS_MAXDATA) {
        fprintf(stderr,
                "serve_nfs_procedure_8_write_to_file: attempted 'write' of %ld bytes to file at absolute path '%s', "
                "but max write allowed in a single RPC is %d bytes\n",
                writeargs->nfsdata.len, file_absolute_path, NFS_MAXDATA);

        // build the procedure results
        Nfs__AttrStat *attr_stat = create_default_case_attr_stat(
            NFS__STAT__NFSERR_FBIG); // FBIG error is not intended for this, but it's the most similar in meaning
    ...
    if (credential->flavor == RPC__AUTH_FLAVOR__AUTH_SYS) {
        int stat =
            check_write_proc_permissions(file_absolute_path, credential->auth_sys->uid, credential->auth_sys->gid);
        if (stat < 0) {
            fprintf(stderr,
                    "serve_nfs_procedure_8_write_to_file: failed checking WRITE permissions for file at absolute path "
                    "'%s' with error code %d\n",
        ...
        if (stat == 1) {
            ...
            return wrap_procedure_results_in_successful_accepted_reply(attr_stat_size, attr_stat_buffer,
                                                                       "nfs/AttrStat");
        }
    }
    ...
    if (error_code == 4 || error_code == 5 || error_code == 6) {
        ...
        case 4:
            ...
            fprintf(stderr,
                    "serve_nfs_procedure_8_write_to_file: attempted write that would exceed file size limits, to file "
                    "at absolute path '%s'\n",
                    file_absolute_path);
            break;
        case 5:
            ...
            fprintf(stderr,
                    "serve_nfs_procedure_8_write_to_file: physical IO error occurred while trying to write to file at "
                    "absolute path '%s'\n",
                    file_absolute_path);
            break;
        case 6:
            ...
            fprintf(
                stderr,
                "serve_nfs_procedure_8_write_to_file: no space left on device to write to file at absolute path '%s'\n",
                file_absolute_path);
            break;
        }
        ...
        fprintf(stderr,
                "serve_nfs_procedure_8_write_to_file: failed writing to file/directory at absolute path '%s' with "
                "error code %d\n",
                file_absolute_path, error_code);
    ...
    Rpc__AcceptedReply *accepted_reply =
        wrap_procedure_results_in_successful_accepted_reply(attr_stat_size, attr_stat_buffer, "nfs/AttrStat");
