...
pthread_mutex_t readdir_sessions_mutex = PTHREAD_MUTEX_INITIALIZER;     // mutex for protecting concurrent access to any ReadDirSessionsList
...
int read_from_directory(Rpc__AuthSysParams *client_authsysparams, char *directory_absolute_path, ino_t directory_inode_number, ReadDirSessionsList **active_readdir_sessions, long offset_cookie, size_t byte_count, Nfs__DirectoryEntriesList **head, int *end_of_stream) {    
    ...
    do {
        ...
        file_name->filename = strdup(directory_entry->d_name); // make a copy of the file name to persist after this dirent is deallocated by closedir()
