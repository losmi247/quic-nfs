...
void clean_up_nfs_server_threads_list_entry(NfsServerThreadsList *server_threads_list_entry) {
    ...
    switch(server_threads_list_entry->transport_protocol) {
        case TRANSPORT_PROTOCOL_TCP:
            if(server_threads_list_entry->transport_connection.rpc_client_socket_fd == NULL) {
                break;
            }

            close(*server_threads_list_entry->transport_connection.rpc_client_socket_fd);
            free(server_threads_list_entry->transport_connection.rpc_client_socket_fd);
