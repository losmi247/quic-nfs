...
int handle_echo(char *text, char *file_name) {
    ...
    NfsFh__NfsFileHandle start_file_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__FHandle *file_fhandle =
        follow_symbolic_links("echo", rpc_connection_context, filesystem_dag_root, cwd_node, &start_file_fhandle,
                              diropres->diropok->attributes->nfs_ftype->ftype);
    nfs__dir_op_res__free_unpacked(diropres, NULL);
    if (file_fhandle == NULL) {
        return 1;
    }
