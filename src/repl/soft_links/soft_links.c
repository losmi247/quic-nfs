 ...
 */
Nfs__FHandle *resolve_path(char *command_name, RpcConnectionContext *rpc_connection_context,
                           DAGNode *filesystem_dag_root, DAGNode *cwd_node, char *path, Nfs__FType *ftype) {
...
Nfs__FHandle *follow_symbolic_links(char *command_name, RpcConnectionContext *rpc_connection_context,
    ...
    while (curr_file_type == NFS__FTYPE__NFLNK) {
        ...
        Nfs__FHandle *next_file_fhandle = resolve_path(command_name, rpc_connection_context, filesystem_dag_root,
                                                       cwd_node, target_path, &next_file_type);
