...
serialization_library: google_protos rpc_serialization nfs_filehandle_serialization mount_serialization nfs_serialization
...
nfs_filehandle_serialization: ./src/serialization/nfs_fh/nfs_fh.proto
...
# -I flag adds the project root dir to include paths (so that we can include libraries in our files as serialization/mount/mount.pb-c.h e.g.)
	gcc ./src/nfs/server/server.c \
		${serialization_files} ${rpc_program_common_server_files} ${mount_and_nfs_server_files} \
