...
Mount__FhStatus *mount_directory_success(RpcConnectionContext *rpc_connection_context, char *directory_absolute_path) {
    ...
    char *expected_mnt_stat = mount_stat_to_string(MOUNT__STAT__MNT_OK),
         *found_mount_stat = mount_stat_to_string(fhstatus->mnt_status->stat);
    cr_assert_eq(fhstatus->mnt_status->stat, MOUNT__STAT__MNT_OK, "Expected NfsStat %s but got %s", expected_mnt_stat,
                 found_mount_stat);
 ...
 * it creates its own RpcConnectionContext with AUTH_SYS flavor and root uid.
 *
 * The procedure results are validated assuming a non-MOUNT__STAT__MNT_OK Mount status, given in argument
 * 'non_mnt_ok_status'.
 */
void mount_directory_fail(RpcConnectionContext *rpc_connection_context, char *directory_absolute_path,
    ...
    cr_assert_not_null(fhstatus->mnt_status);
    char *expected_mnt_stat = mount_stat_to_string(non_mnt_ok_status),
         *found_mount_stat = mount_stat_to_string(fhstatus->mnt_status->stat);
    cr_assert_eq(fhstatus->mnt_status->stat, non_mnt_ok_status, "Expected NfsStat %s but got %s", expected_mnt_stat,
                 found_mount_stat);
    free(expected_mnt_stat);
    free(found_mount_stat);
...
Nfs__AttrStat *get_attributes_success(RpcConnectionContext *rpc_connection_context, Nfs__FHandle file_fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(attrstat->nfs_status->stat);
    cr_assert_eq(attrstat->nfs_status->stat, NFS__STAT__NFS_OK, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
 ...
 * it creates its own RpcConnectionContext with AUTH_SYS flavor and root uid.
 *
 * The procedure results are validated assuming a non-NFS__STAT__NFS_OK NFS status, given in argument
 * 'non_nfs_ok_status'.
 */
void get_attributes_fail(RpcConnectionContext *rpc_connection_context, Nfs__FHandle file_fhandle,
    ...
    cr_assert_not_null(attrstat->nfs_status);
    char *expected_nfs_stat = nfs_stat_to_string(non_nfs_ok_status),
         *found_nfs_stat = nfs_stat_to_string(attrstat->nfs_status->stat);
    cr_assert_eq(attrstat->nfs_status->stat, non_nfs_ok_status, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
 ...
 * and always call 'nfs__attr_stat__free_unpacked()' on it at some point.
 */
Nfs__AttrStat *set_attributes(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *file_fhandle,
                              Nfs__SAttr *sattr) {
 ...
 */
Nfs__DirOpRes *lookup_file_or_directory(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                                        char *filename) {
...
Nfs__DirOpRes *lookup_file_or_directory_success(RpcConnectionContext *rpc_connection_context,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(diropres->nfs_status->stat);
    cr_assert_eq(diropres->nfs_status->stat, NFS__STAT__NFS_OK, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
...
void lookup_file_or_directory_fail(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(non_nfs_ok_status),
         *found_nfs_stat = nfs_stat_to_string(diropres->nfs_status->stat);
    cr_assert_eq(diropres->nfs_status->stat, non_nfs_ok_status, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
...
Nfs__ReadLinkRes *read_from_symbolic_link_success(RpcConnectionContext *rpc_connection_context,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(readlinkres->nfs_status->stat);
    cr_assert_eq(readlinkres->nfs_status->stat, NFS__STAT__NFS_OK, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
    ...
    cr_assert(strcmp(read_content, expected_target_path) ==
              0); // compare the target path we read from the symlink and the expected target path
...
void read_from_symbolic_link_fail(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *file_fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(non_nfs_ok_status),
         *found_nfs_stat = nfs_stat_to_string(readlinkres->nfs_status->stat);
    cr_assert_eq(readlinkres->nfs_status->stat, non_nfs_ok_status, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
 ...
 */
Nfs__ReadRes *read_from_file(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *file_fhandle, uint32_t offset,
                             uint32_t byte_count) {
...
Nfs__ReadRes *read_from_file_success(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *file_fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(readres->nfs_status->stat);
    cr_assert_eq(readres->nfs_status->stat, NFS__STAT__NFS_OK, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
    ...
    // this is a famous problem - atime is going to be flushed to disk by the kernel only after 24hrs, we can't
    // synchronously update it - so we only check that atime_before <= atime_after
    cr_assert(compare_time(attributes_before_read->atime, read_fattr->atime) <= 0,
              "Access time before read is %ld sec %ld nanosec and is larger than access time after read %ld sec %ld "
              "nanosec\n",
              attributes_before_read->atime->seconds, attributes_before_read->atime->useconds,
              read_fattr->atime->seconds, read_fattr->atime->useconds);             // file was accessed
    ...
    cr_assert_eq(read_content.len, expected_read_size, "Expected to read %d bytes but read %ld bytes",
                 expected_read_size, read_content.len);

    cr_assert(memcmp(read_content.data, expected_read_content, expected_read_size) ==
              0); // compare the buffer we read and the expected buffer
...
void read_from_file_fail(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *file_fhandle, uint32_t offset,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(non_nfs_ok_status),
         *found_nfs_stat = nfs_stat_to_string(readres->nfs_status->stat);
    cr_assert_eq(readres->nfs_status->stat, non_nfs_ok_status, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
 ...
 * and always call 'nfs__attr_stat__free_unpacked()' on it at some point.
 */
Nfs__AttrStat *write_to_file(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *file_fhandle, uint32_t offset,
                             uint32_t byte_count, uint8_t *source_buffer) {
 ...
 * and always call 'nfs__dir_op_res__free_unpacked()' on it at some point.
 */
Nfs__DirOpRes *create_file(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                           char *filename, Nfs__SAttr *sattr) {
    Nfs__FileName file_name = NFS__FILE_NAME__INIT;
    file_name.filename = filename;
 ...
 */
Nfs__NfsStat *remove_file(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                          char *filename) {
    Nfs__FileName file_name = NFS__FILE_NAME__INIT;
    file_name.filename = filename;
...
Nfs__NfsStat *remove_file_success(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(nfsstat->stat);
 ...
 * and always call 'nfs__nfs_stat__free_unpacked()' on it at some point.
 */
Nfs__NfsStat *rename_file(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *from_directory_fhandle,
...
Nfs__NfsStat *rename_file_success(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *from_directory_fhandle,
    ...
    Nfs__NfsStat *nfsstat =
        rename_file(rpc_connection_context, from_directory_fhandle, from_filename, to_directory_fhandle, to_filename);
    if (created_rpc_connection_context) {
        free_rpc_connection_context(rpc_connection_context);
    }
 ...
 * and always call 'nfs__nfs_stat__free_unpacked()' on it at some point.
 */
Nfs__NfsStat *create_link_to_file(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *target_file,
                                  Nfs__FHandle *directory_fhandle, char *filename) {
    Nfs__FileName file_name = NFS__FILE_NAME__INIT;
    file_name.filename = filename;
 ...
 * and always call 'nfs__nfs_stat__free_unpacked()' on it at some point.
 */
Nfs__NfsStat *create_symbolic_link(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                                   char *filename, Nfs__Path *target) {
    Nfs__FileName file_name = NFS__FILE_NAME__INIT;
    file_name.filename = filename;
    ...
    Nfs__SAttr sattr =
        NFS__SATTR__INIT; // Unix-like NFS servers ignore 'attributes' for symbolic links, so just put -1 everywhere
    sattr.mode = -1;
 ...
 * and always call 'nfs__dir_op_res__free_unpacked()' on it at some point.
 */
Nfs__DirOpRes *create_directory(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                                char *filename, Nfs__SAttr *sattr) {
    Nfs__FileName file_name = NFS__FILE_NAME__INIT;
    file_name.filename = filename;
 ...
 * and always call 'nfs__nfs_stat__free_unpacked()' on it at some point.
 */
Nfs__NfsStat *remove_directory(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                               char *filename) {
    Nfs__FileName file_name = NFS__FILE_NAME__INIT;
    file_name.filename = filename;
 ...
 */
Nfs__ReadDirRes *read_from_directory(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
                                     uint64_t cookie, uint32_t byte_count) {
...
Nfs__ReadDirRes *read_from_directory_success(RpcConnectionContext *rpc_connection_context,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(readdirres->nfs_status->stat);
    cr_assert_eq(readdirres->nfs_status->stat, NFS__STAT__NFS_OK, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
    ...
    for (int i = 0; i < expected_number_of_entries; i++) {
        ...
        cr_assert(filename_found == 1, "Entry '%s' in procedure results is not among expected directory entries",
                  filename);
...
void read_from_directory_fail(RpcConnectionContext *rpc_connection_context, Nfs__FHandle *directory_fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(non_nfs_ok_status),
         *found_nfs_stat = nfs_stat_to_string(readdirres->nfs_status->stat);
    cr_assert_eq(readdirres->nfs_status->stat, non_nfs_ok_status, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
...
Nfs__StatFsRes *get_filesystem_attributes_success(RpcConnectionContext *rpc_connection_context, Nfs__FHandle fhandle) {
    ...
    char *expected_nfs_stat = nfs_stat_to_string(NFS__STAT__NFS_OK),
         *found_nfs_stat = nfs_stat_to_string(statfsres->nfs_status->stat);
    cr_assert_eq(statfsres->nfs_status->stat, NFS__STAT__NFS_OK, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
...
void get_filesystem_attributes_fail(RpcConnectionContext *rpc_connection_context, Nfs__FHandle fhandle,
    ...
    char *expected_nfs_stat = nfs_stat_to_string(non_nfs_ok_status),
         *found_nfs_stat = nfs_stat_to_string(statfsres->nfs_status->stat);
    cr_assert_eq(statfsres->nfs_status->stat, non_nfs_ok_status, "Expected NfsStat %s but got %s", expected_nfs_stat,
                 found_nfs_stat);
    free(expected_nfs_stat);
    free(found_nfs_stat);
