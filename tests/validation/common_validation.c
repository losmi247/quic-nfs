...
void validate_fattr(Nfs__FAttr *fattr, Nfs__FType ftype) {
    ...
    char *expected_file_type = file_type_to_string(ftype),
         *found_file_type = file_type_to_string(fattr->nfs_ftype->ftype);
    cr_assert_eq(fattr->nfs_ftype->ftype, ftype, "Expected file type %s but found file type %s", expected_file_type,
                 found_file_type);
...
void check_equal_fattr(Nfs__FAttr *fattr1, Nfs__FAttr *fattr2) {
    ...
    // cr_assert_eq(fattr1->nlink, fattr2->nlink); - do not test this, if LINK procedure was done recently, number of
    // links may be updated asynchronously
...
void check_fattr_update(Nfs__FAttr *updated_fattr, Nfs__SAttr *sattr) {
    ...
    if (sattr->mode != -1) {
        cr_assert_eq(((updated_fattr->mode) & ((1 << 12) - 1)),
                     sattr->mode); // only check first 12 bits - (sticky,setuid,setgid),owner,group,others, before that
                                   // is device/file type
    }
