...
Test(nfs_lookup_test_suite, lookup_no_read_permission_on_containing_directory, .description = "NFSPROC_LOOKUP no read permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    // now try to lookup a file 'file.txt' inside this /nfs_share/permission_test/only_owner_read directory, without having read permission
    ...
    // fail since you don't have read permission on containing directory
    lookup_file_or_directory_fail(rpc_connection_context, &only_owner_read_fhandle, "file.txt", NFS__STAT__NFSERR_ACCES);

    free_rpc_connection_context(rpc_connection_context);
}

Test(nfs_lookup_test_suite, lookup_has_read_permission_on_containing_directory, .description = "NFSPROC_LOOKUP has read permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    // now lookup the file 'file.txt' inside this /nfs_share/permission_test/only_owner_read directory
    Nfs__FHandle only_owner_read_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle only_owner_read_nfs_filehandle_copy = deep_copy_nfs_filehandle(only_owner_read_dir_diropres->diropok->file->nfs_filehandle);
    nfs__dir_op_res__free_unpacked(only_owner_read_dir_diropres, NULL);
    only_owner_read_fhandle.nfs_filehandle = &only_owner_read_nfs_filehandle_copy;
    ...
    Nfs__DirOpRes *diropres = lookup_file_or_directory_success(rpc_connection_context, &only_owner_read_fhandle, "file.txt", NFS__FTYPE__NFREG);
    nfs__dir_op_res__free_unpacked(diropres, NULL);
