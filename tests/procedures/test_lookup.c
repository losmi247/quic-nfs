...
Test(nfs_lookup_test_suite, lookup_ok, .description = "NFSPROC_LOOKUP ok") {
    ...
    Nfs__DirOpRes *diropres =
        lookup_file_or_directory_success(rpc_connection_context, &dir_fhandle, "test_file.txt", NFS__FTYPE__NFREG);
...
Test(nfs_lookup_test_suite, lookup_inside_non_existent_directory,
     .description = "NFSPROC_LOOKUP inside non-existent directory") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
...
Test(nfs_lookup_test_suite, lookup_no_such_file_or_directory,
     .description = "NFSPROC_LOOKUP no such file or directory") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
...
Test(nfs_lookup_test_suite, lookup_no_execute_permission_on_containing_directory,
     .description = "NFSPROC_LOOKUP no execute permission on containing directory") {
    ...
    Nfs__DirOpRes *only_owner_execute_dir_diropres =
        lookup_file_or_directory_success(NULL, &permission_test_fhandle, "only_owner_execute", NFS__FTYPE__NFDIR);

    // now try to lookup a file 'file.txt' inside this /nfs_share/permission_test/only_owner_execute directory, without
    // having execute permission
    Nfs__FHandle only_owner_execute_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle only_owner_execute_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(only_owner_execute_dir_diropres->diropok->file->nfs_filehandle);
    ...
    lookup_file_or_directory_fail(rpc_connection_context, &only_owner_execute_fhandle, "file.txt",
                                  NFS__STAT__NFSERR_ACCES);

    free_rpc_connection_context(rpc_connection_context);
}

Test(nfs_lookup_test_suite, lookup_has_execute_permission_on_containing_directory,
     .description = "NFSPROC_LOOKUP has execute permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    Nfs__DirOpRes *diropres = lookup_file_or_directory_success(rpc_connection_context, &only_owner_execute_fhandle,
                                                               "file.txt", NFS__FTYPE__NFREG);
    nfs__dir_op_res__free_unpacked(diropres, NULL);
