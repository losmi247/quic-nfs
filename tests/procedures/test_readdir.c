...
Test(nfs_readdir_test_suite, readdir_ok, .description = "NFSPROC_READDIR ok") {
    ...
    Nfs__ReadDirRes *readdirres = read_from_directory_success(rpc_connection_context, &fhandle, 0, 1000,
...
Test(nfs_readdir_test_suite, readdir_no_such_directory, .description = "NFSPROC_READDIR no such directory") {
    ...
    read_from_directory_fail(rpc_connection_context, &fhandle, 0, 1000,
                             NFS__STAT__NFSERR_NOENT); // cookie=0, start from beginning of the directory stream
...
Test(nfs_readdir_test_suite, readdir_not_a_directory, .description = "NFSPROC_READDIR not a directory") {
    ...
    read_from_directory_fail(rpc_connection_context, &file_fhandle, 0, 1000,
                             NFS__STAT__NFSERR_NOTDIR); // cookie=0, start from beginning of the directory stream
...
Test(nfs_readdir_test_suite, readdir_ok_lookup_then_readdir, .description = "NFSPROC_READDIR ok lookup then readdir") {
    ...
    Nfs__ReadDirRes *readdirres = read_from_directory_success(rpc_connection_context, &dir_fhandle, 0, 1000,
                                                              expected_number_of_entries, expected_filenames);
...
Test(nfs_readdir_test_suite, readdir_ok_read_only_first_directory_entry,
     .description = "NFSPROC_READDIR ok read only first two directory entries") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    Nfs__ReadDirRes *readdirres = read_from_directory_success(rpc_connection_context, &fhandle, 0, 40,
                                                              expected_number_of_entries, expected_filenames);
...
Test(nfs_readdir_test_suite, readdir_ok_read_directory_entries_in_batches,
     .description = "NFSPROC_READDIR ok read directory entries in batches") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    while (eof != 1 && directory_entries_seen < expected_number_of_entries) {
        ...
        while (directory_entries_head != NULL && directory_entries_seen < expected_number_of_entries) {
            ...
            // update the cookie so that the next NFSPROC_READDIR call starts reading directories from where the
...
Test(nfs_readdir_test_suite, readdir_no_read_permission, .description = "NFSPROC_READDIR no read permission") {
    ...
    // now try to read entries in this directory /nfs_share/permission_test/only_owner_read, without having execute
    // permission on it
...
Test(nfs_readdir_test_suite, readdir_has_read_permission_on_containing_directory,
     .description = "NFSPROC_READDIR has read permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    Nfs__ReadDirRes *readdires = read_from_directory_success(rpc_connection_context, &only_owner_read_fhandle, 0, 1000,
                                                             expected_number_of_entries, expected_entries);
