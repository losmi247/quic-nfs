...
Test(nfs_link_test_suite, link_ok, .description = "NFSPROC_LINK ok") {
    ...
    Nfs__DirOpRes *link_test_dir_diropres =
        lookup_file_or_directory_success(rpc_connection_context, &fhandle, "link_test", NFS__FTYPE__NFDIR);

    // lookup the 'existing_file' file inside this link_test directory
    Nfs__FHandle link_test_dir_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle link_test_dir_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(link_test_dir_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__DirOpRes *target_file_diropres = lookup_file_or_directory_success(
        rpc_connection_context, &link_test_dir_fhandle, "existing_file", NFS__FTYPE__NFREG);
    ...
    NfsFh__NfsFileHandle target_file_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(target_file_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__NfsStat *nfsstat =
        create_link_to_file_success(rpc_connection_context, &target_file_fhandle, &link_test_dir_fhandle, "hardlink");
    ...
    Nfs__DirOpRes *link_diropres =
        lookup_file_or_directory_success(rpc_connection_context, &link_test_dir_fhandle, "hardlink", NFS__FTYPE__NFREG);

    // read from the created hard link to confirm it works
    Nfs__FHandle link_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle link_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(link_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__ReadRes *readres =
        read_from_file_success(rpc_connection_context, &link_fhandle, 0, expected_read_size, attributes_before_read,
...
Test(nfs_link_test_suite, link_no_such_target_file, .description = "NFSPROC_LINK no such target file") {
    ...
    create_link_to_file_fail(rpc_connection_context, &target_file_fhandle, &link_test_dir_fhandle, "hardlink",
...
Test(nfs_link_test_suite, link_target_file_is_a_directory, .description = "NFSPROC_LINK target file is a directory") {
    ...
    Nfs__DirOpRes *target_file_diropres = lookup_file_or_directory_success(
        rpc_connection_context, &link_test_dir_fhandle, "existing_directory", NFS__FTYPE__NFDIR);
...
Test(nfs_link_test_suite, link_no_such_directory, .description = "NFSPROC_LINK no such directory") {
    ...
    create_link_to_file_fail(rpc_connection_context, &target_file_fhandle, &dir_fhandle, "hardlink",
                             NFS__STAT__NFSERR_NOENT);

    free_rpc_connection_context(rpc_connection_context);
}

Test(nfs_link_test_suite, link_create_a_hard_link_in_a_non_directory,
     .description = "NFSPROC_LINK create hard link in a non-directory") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    create_link_to_file_fail(rpc_connection_context, &target_file_fhandle, &file_fhandle, "hardlink",
...
Test(nfs_link_test_suite, create_hard_link_that_is_an_already_existing_file,
     .description = "NFSPROC_LINK create hard link that is an already existing file") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    create_link_to_file_fail(rpc_connection_context, &target_file_fhandle, &link_test_dir_fhandle, "existing_file",
...
Test(nfs_link_test_suite, link_no_write_permission_on_containing_directory,
     .description = "NFSPROC_LINK no write permission on containing directory") {
    ...
    // lookup the link_test directory inside the mounted directory
    Nfs__DirOpRes *link_test_dir_diropres =
        lookup_file_or_directory_success(NULL, &fhandle, "link_test", NFS__FTYPE__NFDIR);
    ...
    Nfs__DirOpRes *target_file_diropres =
        lookup_file_or_directory_success(NULL, &link_test_dir_fhandle, "existing_file", NFS__FTYPE__NFREG);

    // now try to create a hard link inside this /nfs_share/permission_test/only_owner_write directory, without having
    ...
    create_link_to_file_fail(rpc_connection_context, &target_file_fhandle, &only_owner_write_fhandle,
                             NONEXISTENT_FILENAME, NFS__STAT__NFSERR_ACCES);

    free_rpc_connection_context(rpc_connection_context);
}

Test(nfs_link_test_suite, link_has_write_permission_on_containing_directory,
     .description = "NFSPROC_LINK has write permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    Nfs__NfsStat *nfsstat = create_link_to_file_success(rpc_connection_context, &target_file_fhandle,
                                                        &only_owner_write_fhandle, "permissions_hardlink");
    nfs__nfs_stat__free_unpacked(nfsstat, NULL);
    ...
    // lookup the created 'hardlink' file to verify its file type is NFREG, same as the target
    Nfs__DirOpRes *link_diropres =
        lookup_file_or_directory_success(NULL, &only_owner_write_fhandle, "permissions_hardlink", NFS__FTYPE__NFREG);
    ...
    uint8_t *expected_new_test_file_content = "existing_file_content";
    uint8_t expected_read_size = strlen(expected_new_test_file_content);
    Nfs__ReadRes *readres = read_from_file_success(NULL, &link_fhandle, 0, expected_read_size, attributes_before_read,
                                                   expected_read_size, expected_new_test_file_content);
