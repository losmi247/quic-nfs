...
Test(nfs_create_test_suite, create_ok, .description = "NFSPROC_CREATE ok") {
    ...
    Nfs__DirOpRes *create_test_dir_diropres =
        lookup_file_or_directory_success(rpc_connection_context, &fhandle, "create_test", NFS__FTYPE__NFDIR);
    ...
    NfsFh__NfsFileHandle create_test_dir_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(create_test_dir_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__DirOpRes *diropres =
        create_file_success(rpc_connection_context, &create_test_dir_fhandle, "create_test_file.txt", 0440, 0, 0, 10,
                            &atime, &mtime, NFS__FTYPE__NFREG); // root-owned file (uid=gid=0)
...
Test(nfs_create_test_suite, create_no_such_directory, .description = "NFSPROC_CREATE no such directory") {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(rpc_connection_context, &fhandle, "create_test.txt", 0440, 0, 0, 10, &atime, &mtime,
...
Test(nfs_create_test_suite, create_file_in_a_non_directory,
     .description = "NFSPROC_CREATE create file in a non-directory") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(rpc_connection_context, &file_fhandle, "create_test.txt", 0440, 0, 0, 10, &atime, &mtime,
...
Test(nfs_create_test_suite, create_already_existing_file,
     .description = "NFSPROC_CREATE create already existing file") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(rpc_connection_context, &create_test_dir_fhandle, "existing_file.txt", 0440, 0, 0, 10, &atime,
...
Test(nfs_create_test_suite, create_no_write_permission_on_containing_directory,
     .description = "NFSPROC_CREATE no write permission on containing directory") {
    ...
    // now try to create a file inside this /nfs_share/permission_test/only_owner_write directory, without having write
    ...
    create_file_fail(rpc_connection_context, &only_owner_write_fhandle, NONEXISTENT_FILENAME, 0, 0, 0, 0, &atime,
                     &mtime, NFS__STAT__NFSERR_ACCES);

    free_rpc_connection_context(rpc_connection_context);
}

Test(nfs_create_test_suite, create_has_write_permission_on_containing_directory,
     .description = "NFSPROC_CREATE has write permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    Nfs__DirOpRes *diropres = create_file_success(rpc_connection_context, &only_owner_write_fhandle, "create_file.txt",
                                                  0, 0, 0, 0, &atime, &mtime, NFS__FTYPE__NFREG);
    nfs__dir_op_res__free_unpacked(diropres, NULL);
