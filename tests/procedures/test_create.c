...
Test(nfs_create_test_suite, create_ok, .description = "NFSPROC_CREATE ok") {
    ...
    Nfs__DirOpRes *create_test_dir_diropres = lookup_file_or_directory_success(NULL, &fhandle, "create_test", NFS__FTYPE__NFDIR);
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    Nfs__DirOpRes *diropres = create_file_success(NULL, &create_test_dir_fhandle, "create_test_file.txt", 0440, 0, 0, 10, &atime, &mtime, NFS__FTYPE__NFREG); // root-owned file (uid=gid=0)
...
Test(nfs_create_test_suite, create_no_such_directory, .description = "NFSPROC_CREATE no such directory") {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(NULL, &fhandle, "create_test.txt", 0440, 0, 0, 10, &atime, &mtime, NFS__STAT__NFSERR_NOENT);
...
Test(nfs_create_test_suite, create_file_in_a_non_directory, .description = "NFSPROC_CREATE create file in a non-directory") {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(NULL, &file_fhandle, "create_test.txt", 0440, 0, 0, 10, &atime, &mtime, NFS__STAT__NFSERR_NOTDIR);
}
...
Test(nfs_create_test_suite, create_file_name_too_long, .description = "NFSPROC_CREATE file name too long") {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(NULL, &create_test_dir_fhandle, filename, 0440, 0, 0, 10, &atime, &mtime, NFS__STAT__NFSERR_NAMETOOLONG);
}
...
Test(nfs_create_test_suite, create_already_existing_file, .description = "NFSPROC_CREATE create already existing file") {
    ...
    mtime.seconds = 200;
    mtime.useconds = 0;
    create_file_fail(NULL, &create_test_dir_fhandle, "existing_file.txt", 0440, 0, 0, 10, &atime, &mtime, NFS__STAT__NFSERR_EXIST);
}
