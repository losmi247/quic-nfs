...
Test(nfs_readlink_test_suite, readlink_ok, .description = "NFSPROC_READLINK ok") {
    ...
    Nfs__DirOpRes *readlink_test_dir_diropres =
        lookup_file_or_directory_success(rpc_connection_context, &fhandle, "readlink_test", NFS__FTYPE__NFDIR);
    ...
    NfsFh__NfsFileHandle readlink_test_dir_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(readlink_test_dir_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__DirOpRes *symlink_diropres = lookup_file_or_directory_success(
        rpc_connection_context, &readlink_test_dir_fhandle, "symlink", NFS__FTYPE__NFLNK);
    ...
    Nfs__ReadLinkRes *readlinkres =
        read_from_symbolic_link_success(rpc_connection_context, &symlink_fhandle, expected_symlink_target_path);
...
Test(nfs_readlink_test_suite, readlink_not_a_symboliclink,
     .description = "NFSPROC_READLINK specified file which is not a symbolic link") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
        cr_fatal("readlink_no_such_file: Failed to connect to the server\n");
    }
