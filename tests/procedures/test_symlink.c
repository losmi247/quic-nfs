...
Test(nfs_symlink_test_suite, symlink_ok, .description = "NFSPROC_SYMLINK ok") {
    ...
    Nfs__DirOpRes *symlink_test_dir_diropres =
        lookup_file_or_directory_success(rpc_connection_context, &fhandle, "symlink_test", NFS__FTYPE__NFDIR);

    // create a symbolic link 'symlink' to /nfs_share/symlink_test/existing_file, inside this /nfs_share/symlink_test
    // directory
    Nfs__FHandle symlink_test_dir_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle symlink_test_dir_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(symlink_test_dir_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__NfsStat *nfsstat =
        create_symbolic_link_success(rpc_connection_context, &symlink_test_dir_fhandle, "symlink", &target);
    nfs__nfs_stat__free_unpacked(nfsstat, NULL);

    // lookup the created 'symlink' file to verify its file type is NFLNK
    Nfs__DirOpRes *symlink_diropres = lookup_file_or_directory_success(
        rpc_connection_context, &symlink_test_dir_fhandle, "symlink", NFS__FTYPE__NFLNK);
    ...
    NfsFh__NfsFileHandle symlink_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(symlink_diropres->diropok->file->nfs_filehandle);
    ...
    Nfs__ReadLinkRes *readlinkres =
        read_from_symbolic_link_success(rpc_connection_context, &symlink_fhandle, target.path);
...
Test(nfs_symlink_test_suite, symlink_create_a_symbolic_link_in_a_non_directory,
     .description = "NFSPROC_SYMLINK create symbolic link in a non-directory") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
...
Test(nfs_symlink_test_suite, symlink_file_name_too_long, .description = "NFSPROC_SYMLINK file name too long") {
    ...
    // try to create a symbolic link (to some target) with a too long filename inside this /nfs_share/symlink_test
    // directory
    Nfs__FHandle symlink_test_dir_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle symlink_test_dir_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(symlink_test_dir_diropres->diropok->file->nfs_filehandle);
    ...
    create_symbolic_link_fail(rpc_connection_context, &symlink_test_dir_fhandle, filename, &path,
...
Test(nfs_symlink_test_suite, create_symbolic_link_that_is_an_already_existing_file,
     .description = "NFSPROC_SYMLINK create symbolic link that is an already existing file") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
    ...
    create_symbolic_link_fail(rpc_connection_context, &symlink_test_dir_fhandle, "existing_file", &path,
...
Test(nfs_symlink_test_suite, symlink_no_write_permission_on_containing_directory,
     .description = "NFSPROC_SYMLINK no write permission on containing directory") {
    ...
    // now try to create a symbolic link inside this /nfs_share/permission_test/only_owner_write directory, without
    // having write permission on that directory
    ...
    create_symbolic_link_fail(rpc_connection_context, &only_owner_write_fhandle, NONEXISTENT_FILENAME, &path,
                              NFS__STAT__NFSERR_ACCES);

    free_rpc_connection_context(rpc_connection_context);
}

Test(nfs_symlink_test_suite, symlink_has_write_permission_on_containing_directory,
     .description = "NFSPROC_SYMLINK has write permission on containing directory") {
    Mount__FhStatus *fhstatus = mount_directory_success(NULL, "/nfs_share");
    ...
    Nfs__NfsStat *nfsstat =
        create_symbolic_link_success(rpc_connection_context, &only_owner_write_fhandle, "permissions_symlink", &path);
    nfs__nfs_stat__free_unpacked(nfsstat, NULL);

    free_rpc_connection_context(rpc_connection_context);

    // lookup the created 'permissions_symlink' file to verify its file type is NFLNK
    Nfs__DirOpRes *symlink_diropres =
        lookup_file_or_directory_success(NULL, &only_owner_write_fhandle, "permissions_symlink", NFS__FTYPE__NFLNK);
