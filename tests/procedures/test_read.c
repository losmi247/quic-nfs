...
Test(nfs_read_test_suite, read_ok, .description = "NFSPROC_READ ok") {
    ...
    Nfs__ReadRes *readres = read_from_file_success(NULL, &file_fhandle, 2, expected_read_size, diropres->diropok->attributes, expected_read_size, expected_read_content);
...
Test(nfs_read_test_suite, read_no_such_file, .description = "NFSPROC_READ no such file") {
    ...
    read_from_file_fail(NULL, &file_fhandle, 2, 10, NFS__STAT__NFSERR_NOENT);
...
Test(nfs_read_test_suite, read_is_directory, .description = "NFSPROC_READ directory specified for a non-directory operation") {
    ...
    read_from_file_fail(NULL, &fhandle, 2, 10, NFS__STAT__NFSERR_ISDIR);
}
...
Test(nfs_read_test_suite, read_more_than_maxdata_bytes, .description = "NFSPROC_READ read more than maxdata bytes") {
    ...
    Nfs__DirOpRes *diropres = lookup_file_or_directory_success(NULL, &fhandle, "large_file.txt", NFS__FTYPE__NFREG);
    ...
    Nfs__ReadRes *readres = read_from_file_success(NULL, &file_fhandle, 0, NFS_MAXDATA + 20, diropres->diropok->attributes, NFS_MAXDATA, expected_read_content);
    nfs__dir_op_res__free_unpacked(diropres, NULL);
...
Test(nfs_read_test_suite, read_no_read_permission, .description = "NFSPROC_READ no read permission") {
    ...
    Nfs__DirOpRes *only_owner_read_file_diropres = lookup_file_or_directory_success(NULL, &permission_test_fhandle, "only_owner_read.txt", NFS__FTYPE__NFREG);
    ...
    NfsFh__NfsFileHandle only_owner_read_nfs_filehandle_copy = deep_copy_nfs_filehandle(only_owner_read_file_diropres->diropok->file->nfs_filehandle);
...
Test(nfs_read_test_suite, read_has_read_permission, .description = "NFSPROC_READ has read permission") {
    ...
    Nfs__ReadRes *readres = read_from_file_success(rpc_connection_context, &only_owner_read_fhandle, 0, 8, only_owner_read_file_diropres->diropok->attributes, 8, "testdata");
    nfs__read_res__free_unpacked(readres, NULL);
