...
Test(nfs_read_test_suite, read_ok, .description = "NFSPROC_READ ok") {
    ...
    Nfs__ReadRes *readres = read_from_file_success(rpc_connection_context, &file_fhandle, 2, expected_read_size, diropres->diropok->attributes, expected_read_size, expected_read_content);
...
Test(nfs_read_test_suite, read_more_than_maxdata_bytes, .description = "NFSPROC_READ read more than maxdata bytes") {
    ...
    Nfs__DirOpRes *diropres = lookup_file_or_directory_success(rpc_connection_context, &fhandle, "large_file.txt", NFS__FTYPE__NFREG);
    ...
    Nfs__ReadRes *readres = read_from_file_success(rpc_connection_context, &file_fhandle, 0, NFS_MAXDATA + 20, diropres->diropok->attributes, NFS_MAXDATA, expected_read_content);
    nfs__dir_op_res__free_unpacked(diropres, NULL);
