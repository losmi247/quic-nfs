...
Test(nfs_setattr_test_suite, setattr_ok, .description = "NFSPROC_SETATTR ok") {
    ...
    Nfs__AttrStat *attrstat =
        set_attributes_success(rpc_connection_context, &fhandle, 0, 0, 0, -1, &atime, &mtime, NFS__FTYPE__NFDIR);
...
Test(nfs_setattr_test_suite, setattr_no_such_file_or_directory,
     .description = "NFSPROC_SETATTR no such file or directory") {
    RpcConnectionContext *rpc_connection_context = create_test_rpc_connection_context(TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
...
Test(nfs_setattr_test_suite, setattr_change_ownership_without_root_privileges,
     .description = "NFSPROC_SETATTR change ownership without root privileges") {
    ...
    // try to change ownership of this /nfs_share directory without root privileges (only root should be able to change
    // ownership of files)
    ...
    RpcConnectionContext *rpc_connection_context =
        create_rpc_connection_context_with_test_ipaddr_and_port(non_root_credential, verifier, TEST_TRANSPORT_PROTOCOL);
    if (rpc_connection_context == NULL) {
        cr_fatal("setattr_no_such_file_or_directory: Failed to connect to the server\n");
    }
...
Test(nfs_setattr_test_suite, setattr_no_write_permission, .description = "NFSPROC_SETATTR no write permission") {
    ...
    Nfs__DirOpRes *setattr_only_owner_write_file_diropres = lookup_file_or_directory_success(
        NULL, &permission_test_fhandle, "setattr_only_owner_write.txt", NFS__FTYPE__NFREG);

    // now try to update attributes of this 'only_owner_write.txt' (other than uid/gid, so that we don't change
    // ownership i.e. don't need root privileges)
    Nfs__FHandle setattr_only_owner_write_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle setattr_only_owner_write_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(setattr_only_owner_write_file_diropres->diropok->file->nfs_filehandle);
    ...
    set_attributes_fail(rpc_connection_context, &setattr_only_owner_write_fhandle, -1, -1, -1, -1, &atime, &mtime,
...
Test(nfs_setattr_test_suite, setattr_has_write_permission, .description = "NFSPROC_SETATTR has write permission") {
    ...
    // update attributes of this 'setattr_only_owner_write.txt' (other than uid/gid, so that we don't change ownership
    // i.e. don't need root privileges, only write permission)
    Nfs__FHandle setattr_only_owner_write_fhandle = NFS__FHANDLE__INIT;
    NfsFh__NfsFileHandle setattr_only_owner_write_nfs_filehandle_copy =
        deep_copy_nfs_filehandle(setattr_only_owner_write_file_diropres->diropok->file->nfs_filehandle);
    nfs__dir_op_res__free_unpacked(setattr_only_owner_write_file_diropres, NULL);
    ...
    Nfs__AttrStat *attrstat = set_attributes_success(rpc_connection_context, &setattr_only_owner_write_fhandle, -1, -1,
                                                     -1, -1, &atime, &mtime, NFS__FTYPE__NFREG);
    nfs__attr_stat__free_unpacked(attrstat, NULL);
